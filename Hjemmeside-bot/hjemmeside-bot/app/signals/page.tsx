            ...
            <div className="min-h-screen" style={{ background: "#211d1d" }}>
                <div className="mx-auto max-w-5xl px-4 py-10">
                    <div className="rounded-2xl p-6" style={{ background: "#2a2727", color: "#f0f0f0" }}>
                        ...
                        <Link href="/signup" className="underline" style={{ color: gold }}>
                            Gå til login
                        </Link>
                    </div>
                </div>
              ...
              {plan.toUpperCase()}
                ...
                <section className="rounded-2xl p-5 space-y-5 border" style={{ background: "#2a2727", color: "#f0f0f0", borderColor: border }}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        ...
                        {/* tom kolonne for bedre spacing på 3-kol grid */}
                        <div />
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <label className="text-sm">
                            ...
                            <input
                                inputMode="decimal"
                                value={fmt(draft.entry)}
                                onChange={(e) => setDraft((d) => ({ ...d, entry: parseNum(e.target.value) }))}
                                placeholder="1.2345"
                                className="w-full rounded-md px-3 py-2 text-sm outline-none border"
                                style={{ background: "#211d1d", color: "#f0f0f0", borderColor: border }}
                            />
                        ...
                        <label className="text-sm">
                            ...
                            <input
                                inputMode="decimal"
                                value={fmt(draft.stop)}
                                onChange={(e) => setDraft((d) => ({ ...d, stop: parseNum(e.target.value) }))}
                                placeholder="1.2300"
                                className="w-full rounded-md px-3 py-2 text-sm outline-none border"
                                style={{ background: "#211d1d", color: "#f0f0f0", borderColor: border }}
                            />
                                ...
                                <div key={tp.id} className="flex items-center gap-2">
                                    <input
                                        ...
                                        onChange={(e) =>
                                            setDraft((d) => ({
                                                ...d,
                                                tps: d.tps.map((t) => (t.id === tp.id ? { ...t, price: parseNum(e.target.value) } : t)),
                                            }))
                                        }
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between">
                                ...
                                <label className="flex items-center gap-2 text-xs text-gray-300">
                                    <input
                                        type="checkbox"
                                        checked={!!draft.includeTrader}
                                        onChange={(e) => setDraft((d) => ({ ...d, includeTrader: e.target.checked }))}
                                    />
                                    Medtag i besked
                                </label>
                            </div>
                            <div className="flex gap-2">
                                <select
                                    value={draft.traderTag ?? ""}
                                    onChange={(e) => setDraft((d) => ({ ...d, traderTag: e.target.value }))}
                                    className="flex-1 rounded-md px-3 py-2 text-sm outline-none border"
                                    style={{ background: "#211d1d", color: "#f0f0f0", borderColor: border }}
                                >
                                    {defaultTraders.map((t) => <option key={t} value={t}>{t}</option>)}
                                    <option value="__custom__">✏️ Nyt navn…</option>
                                </select>
                                {draft.traderTag === "__custom__" && (
                                    <input
                                        autoFocus
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between">
                                ...
                                <label className="flex items-center gap-2 text-xs text-gray-300">
                                    <input
                                        type="checkbox"
                                        checked={!!draft.includeStrategy}
                                        onChange={(e) => setDraft((d) => ({ ...d, includeStrategy: e.target.checked }))}
                                    />
                                    Medtag i besked
                                </label>
                            </div>
                            <div className="flex gap-2">
                                <select
                                    ...
                                    onChange={(e) => setDraft((d) => ({ ...d, strategyTag: e.target.value }))}
                                    className="flex-1 rounded-md px-3 py-2 text-sm outline-none border"
                                    style={{ background: "#211d1d", color: "#f0f0f0", borderColor: border }}
                                >
                                    {defaultStrategies.map((t) => <option key={t} value={t}>{t}</option>)}
                                    <option value="__custom__">✏️ Ny strategi…</option>
                                </select>
                                {draft.strategyTag === "__custom__" && (
                                    <input
                                        autoFocus
                    ...
                    {/* Note + Emoji pynt toggle */}
                    <div className="space-y-2">
                        <div className="flex items-center justify-between">
                            <span className="text-sm text-gray-300">Note (valgfri)</span>
                            <label className="flex items-center gap-2 text-xs text-gray-300">
                                <input
                                    type="checkbox"
                                    checked={!!draft.useEmojiDecorations}
                                    onChange={(e) => setDraft((d) => ({ ...d, useEmojiDecorations: e.target.checked }))}
                                />
                                Emoji pynt (📍/❌/🎯)
                            </label>
                        </div>
                        <input
                            ...
                            placeholder="Fx 'TP1 ramt → flyt SL til BE'"
                            className="w-full rounded-md px-3 py-2 text-sm outline-none border"
                            style={{ background: "#211d1d", color: "#f0f0f0", borderColor: border }}
                        />
                    </div>
                                ...
                                <div key={s.id} className="rounded-2xl p-5 border" style={{ background: "#2a2727", borderColor: border, color: "#f0f0f0" }}>
                                    ...
                                    <div className="mt-4 flex flex-wrap items-center gap-2">
                                        {["TP1","TP2","TP3","TP4","TP5","SL"].map((lbl) => (
                                            <button
                                                key={lbl}
                                                onClick={() => markTPorSL(s.id, lbl as any)}
